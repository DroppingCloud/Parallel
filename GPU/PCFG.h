...
#define PARALLEL_THREADSHOLD 30000  // 经验阈值
#define GPU_THRESHOLD  1000
...
public:
    ...
    void GenerateNew(const PT& pt, std::vector<std::string>& guesses);
    ...
    // ========================= 查看猜测分布 =========================
    int last_generated_count = 0;  // 上一次Generate生成的猜测数量
    PT last_pt;                    // 上一次出队处理的PT
    // ========================= 查看猜测分布 =========================
};
...
struct ThreadArgs 
{
    string prefix;      // 前 N-1 个 segment 的实例化字符串
    segment* seg_ptr;   // 指向最后一个 segment 的模型数据（含 ordered_values）
    int start;          // 任务起始位置
    int end;            // 任务终止位置

    vector<string>* guesses;           // 指向全局 guesses 共享容器
    pthread_mutex_t* guess_lock;       // 对 guesses 写入的互斥锁

    int* total_guesses;                // 指向全局 counter 共享计数 
    pthread_mutex_t* counter_lock;     // 对 counter 写入的互斥锁
};