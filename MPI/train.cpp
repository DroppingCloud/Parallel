...
void model::train(string train_path)
{
    /*
    * 给定一个训练集，对模型进行训练
    */
    ...
    while (getline(file, line))
    {
        parse(line);
        lines++;
        if (lines % 10000 == 0 && world_rank == 0) {  // 只有主进程输出进度信息
